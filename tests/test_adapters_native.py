...
from rails import Rails, Message, Role, current_rails
...
with patch('rails.adapters.smolagents.SMOLAGENTS_AVAILABLE', True):
    from rails.adapters.smolagents import SmolAgentsAdapter, WrappedSmolAgent, CodeAgentAdapter
...
class MockLangChainRunnable:
    ...
    def invoke(self, input_data: Any, config=None, **kwargs):
        ...
        return {"content": f"Response to {len(input_data)} messages", "usage_metadata": {"total_tokens": 100}}
    
    async def ainvoke(self, input_data: Any, config=None, **kwargs):
        """Mock asynchronous invoke."""
        self.ainvoke_count += 1
        self.last_input = input_data
        return {"content": f"Async response to {len(input_data)} messages", "usage_metadata": {"total_tokens": 150}}
...
async def test_langchain_wrapper_sync():
    ...
    rails.add_rule(
        ...
        action=lambda msgs: msgs + [Message(role=Role.SYSTEM, content="Injected message")],
        name="inject_test"
    )
...
async def test_langchain_wrapper_async():
    ...
    # Add injection rule
    rails.add_rule(
        condition=LambdaCondition(func=lambda s: s.get_counter_sync("turns") >= 2),
        action=lambda msgs: msgs + [Message(role=Role.SYSTEM, content="Async injection")],
        name="async_inject"
    )
...
async def test_smolagents_wrapper():
    ...
    rails.add_rule(
        condition=LambdaCondition(func=lambda s: s.get_counter_sync("agent_runs") >= 2),
        action=lambda msgs: msgs + [Message(role=Role.SYSTEM, content="Consider breaking down the task")],
    ...
    async with rails:
        ...
        assert "Consider breaking down" in mock_agent.last_task or await rails.store.get_counter("injections") > 0
...
async def test_code_agent_specialized_tracking():
    ...
    rails.add_rule(
        condition=LambdaCondition(func=lambda s: s.get_counter_sync("code_generations") >= 1),
        action=lambda msgs: msgs + [Message(role=Role.SYSTEM, content="Add docstrings to your code")],
...
async def test_complex_injection_scenario():
    ...
    rails.add_rule(
        condition=LambdaCondition(func=lambda s: s.get_counter_sync("turns") == 2),
        action=lambda msgs: msgs + [Message(role=Role.SYSTEM, content="First injection")],
        name="first"
    )
    
    rails.add_rule(
        condition=LambdaCondition(func=lambda s: s.get_counter_sync("turns") == 4),
        action=lambda msgs: msgs + [Message(role=Role.SYSTEM, content="Second injection")],
        name="second"
    )
